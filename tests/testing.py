...
str = """Litwo! Ojczyzno moja! ty jesteś jak zdrowie:
...
Po ścianach: w tej komnacie mieszkanie kobice!
...
I rzekł: Mój Sędzio, dawniej było jeszcze gorzyj!
...
Bo biegła bardzo szybko, suwała się raczyj
...
Bo młodzieńcowi młodą jest każda kobita,
...
Gorzały przeciw sobie jak roratne świce.
...
Tadeusz odpowiadał śmielej, szła rzecz dalyj,
...
listline = str.splitlines()

lastwords = []

for item in listline:
    wordlist = item.split()
    lastwords.append(wordlist[-1])

wordpairs = []

for i, item in enumerate(lastwords):
    if i == 0 or i % 2 == 0:
        try:
            item2 = lastwords[i+3]
        except:
...
for tup in wordpairs:
    ...
    it2 = tup[1]
    print(tup)
    scores.append(Araxne.compare(it1, it2))

print(statistics.fmean(scores))

with open("anti-stats.txt", "a", encoding="utf-8") as f:
        f.write("Para:Wynik\n")

for i, item in enumerate(wordpairs):
    with open("anti-stats.txt", "a", encoding="utf-8") as f:
        f.write(f"{item}:{scores[i]}\n")